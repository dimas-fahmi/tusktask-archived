...
CREATE POLICY "PLC_PROJECT_MASTER_TASKS_ALL_SELF" ON "project"."master_tasks" AS PERMISSIVE FOR ALL TO "authenticated" USING ((select auth.id()) = "project"."master_tasks"."owner_id") WITH CHECK ((select auth.id()) = "project"."master_tasks"."owner_id");--> statement-breakpoint
...
ALTER POLICY "PLC_USER_PROFILES_UPDATE_SELF" ON "user"."profiles" TO authenticated USING ((SELECT auth.id()) = "user"."profiles"."user_id") WITH CHECK ((SELECT auth.id()) = "user"."profiles"."user_id");--> statement-breakpoint
ALTER POLICY "PLC_PROJECT_PROJECTS_ALL_SELF" ON "project"."projects" TO authenticated USING ((select auth.id()) = "project"."projects"."owner_id") WITH CHECK ((select auth.id()) = "project"."projects"."owner_id");